...
import {Plus,Pencil,Trash2,Search,X,ImagePlus,Star,Layers,ChevronLeft,ChevronRight,ChevronsLeft,ChevronsRight,Copy,Clipboard,} from "lucide-react";
...
const Products = () => {
  ...
  const handleKeyDown = (e, currentField, maxLength) => {
    ...
    if (maxLength && e.target.value.length >= maxLength && e.key !== 'Backspace' && e.key !== 'Delete') {
      e.preventDefault();
  ...
  const moveToNextField = (currentField) => {
    const fieldOrder = ['title', 'category', 'description', 'price', 'stock', 'mrp', 'rating'];
  ...
  const handleSubmit = async (e) => {
    ...
    formData.images.forEach((img) => {
      if (img.file) {
        ...
        newImages.push(img);
      } else if (img.preview && !img.file && !img.isExisting) {
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700">
                    ...
                    <span className="text-xs text-gray-500 ml-2">(Press Enter for next)</span>
                  </label>
                  <input
                    ref={titleRef}
                    name="title"
              ...
              <div>
                ...
                >
                  ...
                  <p className="text-sm">Drag & drop, click to upload, or paste images</p>
                  <p className="text-xs text-gray-400 mt-1">Supports: Ctrl+V paste, drag & drop</p>
                </div>
                  ...
                  {formData.images.map((img, i) => (
                    ...
                    >
                      ...
                      <div className="absolute top-0 left-0 right-0 flex justify-between p-1">
                        {img.isExisting && (
                          <span className="text-xs bg-green-500 text-white px-1 rounded">Existing</span>
                        )}
                        <button
                          type="button"
                          onClick={() => copyImageUrl(img.preview)}
